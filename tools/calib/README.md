...
`MainActivity` の `CALIB_CAPTURE = true` でビルド＆インストールすると、起動後に
一定間隔（既定 ~1.2s）で解析フレームを自動保存する（端末を動かすだけでよい）。

- チェスボードを**様々な角度・距離・位置**で写す（正面／左右に傾ける／上下に傾ける）
...
`calib.json` の値を `MainActivity` の `Calib` に転記し、`Calib.enabled = true` にして再ビルド。
